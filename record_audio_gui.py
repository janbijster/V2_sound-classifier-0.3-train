...
from PIL import Image
import sys
from audio_utils import start_recording, end_recording_and_overlap, sample_rate, default_sample_length_sec
...
check_interval = 0.5
default_model_path = 'models/model_with_tire_screech.h5'
default_class_names_file = 'models/class_names_with_tire_screech.json'
...
store_threshold = float(args.threshold)
# get class names
...
def main():
    ...
    while True:
        current_recording = start_recording(trim_recording_start_seconds)
        ...
        for recording_index, recording in enumerate(recordings):
            ...
            if store_sounds:
                if volumes.max() > store_threshold:
                    sound_info = {}
                    ...
                    sound_info['name'] = '{}_{}'.format(datetime_string(recording_index * default_sample_length_sec), sound_info['predicted_class'])
            ...
            if recording_index == len(recordings) - 1:
                update_visualizations(
                    ...
                    store_threshold, default_sample_length_sec,
                    stop_all, start_storing,
                    store_sounds
                )

        recordings = end_recording_and_overlap(current_recording, recordings, trim_recording_start_seconds)
